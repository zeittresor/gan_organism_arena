...
const MODEL_VERSION: String = "arena-biology-11"
...
func model_description() -> Dictionary:
    return {"schema": "arena.observation/1", "model": MODEL_VERSION, "version": "1.0.0-alpha38", "step_seconds": 1.0 / 12.0,
        ...
        "mechanisms": ["persistent exploration and foraging goals with progress-based replanning", "physiology-gated multi-generation shore trials", "head-local nutrient capture and stage-aware food sensing", "compatible-partner approach before courtship", "vegetative plant propagules with substrate dispersal", "optional continuously checked minimum population with random aquatic hatchling founders", "DNA-encoded topology expression, derived morphotypes and heritable macro-mutation", "inherited pigment hue, saturation and brightness", "single-homolog regulatory mutations with allele-change provenance", "event-driven bounded genealogy and separated natural/rescue counts", "ploidy-preserving mitotic tissue growth and repair", "critical unrepaired trauma mortality", "reciprocal meiotic tetrads and haploid gamete pools", "facultative clonal/sexual reproduction", "articulated body contacts", "passive vertical joint conformance and fractional-volume buoyancy", "adjustable model gravity", "load-sensitive support pitch and trailing-joint swimming response", "diploid loci", "linked segregation and recombination", "partial dominance", "recessive genetic load", "paid juvenile development", "finite gamete reserves", "embryo energy and temperature dependence", "predation", "finite food particles with measured external renewal"],
        "assumptions": ["Fictional genotype-to-body map and coefficients; not species-calibrated", "Seven inherited topology grammars can express additional derived multi-gene morphotypes; neither label is a species test", "91-locus regulatory map arena.loci/3; pigment mixing is not a calibrated pigment chemistry model", "Gravity baseline is 9.8 world units/s^2; overlapping structural ellipsoids approximate displaced volume, without fluid dynamics or tissue stress simulation", "Basic animal developmental phases are abstractions, not cell simulation", "Resource renewal, light and ecological food production are external inputs; no closed carbon/nitrogen cycle", "Cognitive/complexity scores are model variables, not validated intelligence measures", "Deterministic stepping applies within this engine/build and configuration; cross-platform floating-point equivalence not promised"],
...
func observation() -> Dictionary:
    ...
    return {"schema": "arena.observation/1", "model": MODEL_VERSION, "seed": world.run_seed, "revision": world.experiment_revision, "step": world.sim_steps, "time": world.elapsed_sim_time, "controlled": world.experiment_mode, "paused": world.simulation_paused,
        "parameters": effective_parameters(), "configuration": world.experiment_settings.duplicate(true), "habitat": {"level": world.habitat.level, "size": world.half_extent * 2.0, "waterline": world.waterline, "bottom": world.habitat.bottom_y, "ceiling": world.habitat.ceiling_y},
        "organisms": items, "broods": broods, "population_metrics": world.metrics(), "births": world.reproduction.births, "conceptions": world.reproduction.conceptions, "brood_losses": world.reproduction.losses,
        "nutrient_energy": world.nutrient_field.stored_energy(), "external_nutrient_input": world.nutrient_input, "event_cursor": world.event_sequence, "evolution_totals": world.evolution_history.totals.duplicate(true)}
...
func effective_parameters() -> Dictionary:
    ...
    result["auto_reseed"] = world._setting("auto_reseed", true)
    return result
...
func organism_data(org, include_genome: bool) -> Dictionary:
    ...
    var item: Dictionary = {"id": org.organism_id, "parents": [org.parent_a, org.parent_b], "family": org.genome.family_id, "generation": org.genome.generation,
        ...
        "behavior": org.behavior_state, "base_plan": org.body_plan_name(), "morphotype": org.morphotype_signature(), "genetic_health": org.genome.genetic_health(), "heterozygosity": org.genome.heterozygosity(), "in_water": org.in_water, "airborne": org.airborne, "rooted": org.rooted}
    ...
    if include_genome:
        ...
        item["genome"] = {"alleles": org.genome.alleles.duplicate(true), "expression": phenotype, "recessive_load": org.genome.recessive_load.duplicate(true), "sex_chromosomes": org.genome.sex_chromosomes.duplicate(), "plan": org.genome.body_plan, "mutations": org.genome.mutation_events, "macro_mutations": org.genome.macro_mutation_events, "mutation_log": org.genome.mutation_log.duplicate(true), "crossovers": org.genome.crossover_events}
    return item

func execute(action: String, arguments: Dictionary) -> Dictionary:
    if world.simulation_paused and action in ["step", "reset", "mode", "parameters"]:
        return {"error": "World paused in the application. Close menus and resume with P, Space or Pause before changing or stepping the world.", "paused": true}
    if action == "describe": return model_description()
    if action == "capture":
        var genomes: Array = []
        for org in world.organisms:
            if is_instance_valid(org) and org.alive: genomes.append(organism_data(org, true))
        return {"model": model_description(), "observation": observation(), "genomes": genomes, "evolution": world.evolution_report()}
    if action == "mode":
    ...
    if action == "parameters" or action == "reset":
        ...
        if action == "reset":
            ...
            if bool(clean["auto_reseed"]) and int(clean["minimum_population"]) > int(clean["organism_cap"]): return {"error": "minimum_population exceeds organism_cap"}
            ...
            if params.has("auto_reseed") or params.has("minimum_population"):
                world.refresh_population_floor()
        return observation()
...
func validate_parameters(params: Dictionary) -> String:
    for key in params:
        ...
        if key in ["initial_organisms", "minimum_population", "organism_cap", "nutrient_count"] and float(value) != floor(float(value)): return str(key) + " must be integral"
    return ""
