...
const MODEL_VERSION: String = "arena-biology-3"
const PARAM_LIMITS: Dictionary = {"mutation_strength": [0.0, 0.5], "macro_mutation_rate": [0.0, 0.2], "nutrient_renewal": [0.0, 4.0], "temperature_offset": [-12.0, 12.0], "predation_strength": [0.0, 1.0], "group_strength": [0.0, 1.0], "initial_organisms": [2.0, 60.0], "organism_cap": [2.0, 80.0], "nutrient_count": [16.0, 1000.0]}
var world = null
...
func model_description() -> Dictionary:
    return {"schema": "arena.observation/1", "model": MODEL_VERSION, "version": "1.0.0-alpha18", "step_seconds": 1.0 / 12.0,
...
func effective_parameters() -> Dictionary:
    ...
    for key in PARAM_LIMITS:
        result[key] = world._setting(key, 1.0 if key == "nutrient_renewal" else 0.0)
...
func organism_data(org, include_genome: bool) -> Dictionary:
    ...
    if include_genome:
        ...
        for i in range(org.visual.body_cells.size()):
            ...
            joints.append({"cell": i, "parent": cell["parent"], "mode": cell["joint_mode"], "axis": [axis.x, axis.y, axis.z], "limit_radians": cell["joint_limit"], "angle_radians": cell["joint_angle"], "muscle": cell["joint_muscle"]})
        item["joints"] = joints
