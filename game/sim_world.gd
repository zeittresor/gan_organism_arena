...
# The seven canonical aquatic body plans are always available. Thirty-five
# additional startup genomes are freshly generated by meiosis/crossover
# mixtures of those plans on every new world. The default population samples
# ten distinct entries from this forty-two-form pool, so each run gets a broad,
# varied but still coherent aquatic starting gene pool.
const STARTUP_FORM_COUNT: int = 42
...
func initialize(seed_value: int = 1337) -> void:
    ...
    # The setting remains adjustable above forty-two; fill any excess with fresh
    ...
    for i in range(organisms.size() - 1, -1, -1):
        ...
        if not is_instance_valid(org) or not org.alive:
            ...
            if is_instance_valid(org):
                ...
                record_event("death", {"id": oid, "age": org.age_seconds, "cause": org.death_cause if not org.death_cause.is_empty() else "unknown", "energy": org.energy, "oxygen": org.oxygen, "offspring": org.children, "genetic_health": org.genome.genetic_health()})
...
func _register_remains(org) -> void:
    ...
    # A body above a submerged floor falls into that water column; a body over
    # land falls directly to the terrestrial surface.
    var aquatic: bool = habitat.floor_at(position) < habitat.waterline
    ...
    var sink_rate: float = 0.22 + mineral * 0.36 + (1.0 - float(org.genome.buoyancy)) * 0.24
    remains.append({"position": position, "size": Traits.body_scale(org.genome), "age": 0.0, "aquatic": aquatic, "plan": org.body_plan_name(), "biomass": biomass, "mineral": mineral,
        "stage": "corpse", "reef_growth": 0.0, "sink_rate": sink_rate, "cause": org.death_cause if not org.death_cause.is_empty() else "unknown"})
...
func _age_remains(dt: float) -> void:
    ...
    for i in range(remains.size() - 1, -1, -1):
        ...
        p.y = maxf(floor_y, p.y - elapsed * (float(item.get("sink_rate", 0.5)) if item["aquatic"] else 4.0))
        item["position"] = p
        var decay: float = minf(float(item["biomass"]), elapsed * 0.008) if float(item["age"]) > 8.0 else 0.0
        ...
        var settled: bool = p.y <= floor_y + 0.001
        if float(item["biomass"]) > 0.35:
            item["stage"] = "corpse" if float(item["age"]) < 28.0 else "carrion"
        elif float(item["mineral"]) > 0.08:
            item["stage"] = "skeleton"
        else:
            item["stage"] = "detritus"
        # Hard submerged remains provide substrate for slow encrusting growth.
        # This is colonisation, not the deceased animal turning into living coral.
        if bool(item["aquatic"]) and settled and float(item["mineral"]) > 0.14 and float(item["biomass"]) < 0.18 and float(item["age"]) > 90.0:
            item["reef_growth"] = minf(1.0, float(item.get("reef_growth", 0.0)) + elapsed * 0.0025 * float(item["mineral"]))
            if float(item["reef_growth"]) > 0.16: item["stage"] = "reef_substrate"
        if float(item["age"]) > 1200.0 and float(item.get("reef_growth", 0.0)) < 0.16:
            remains.remove_at(i)
...
func reset_experiment(seed_value: int, parameters: Dictionary) -> void:
    ...
    for cloud in reproduction.spawn_clouds:
